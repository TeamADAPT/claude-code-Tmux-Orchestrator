...
PREVIOUS_COOLDOWNS=$(redis-cli -p 18000 GET "frosty:cooldown:count" 2>/dev/null || echo "0")
BACKOFF_MULTIPLIER=$((2 ** PREVIOUS_COOLDOWNS))
JITTER=$((RANDOM % 30 + 10))  # 10-40 second jitter
ENHANCED_COOLDOWN=$((BASE_COOLDOWN * BACKOFF_MULTIPLIER + JITTER))

# Cap maximum cooldown at 10 minutes
if [ $ENHANCED_COOLDOWN -gt 600 ]; then
    ENHANCED_COOLDOWN=600
fi
...
if [[ $bang_count -ge $BANG_LIMIT ]]; then
  echo "⚠️ Too many restarts ($bang_count in $BANG_WINDOW sec) — Frosty activating enhanced cooldown: ${ENHANCED_COOLDOWN}s" >&2
  
  # Increment cooldown counter for exponential backoff
  redis-cli -p 18000 INCR "frosty:cooldown:count" >/dev/null
  redis-cli -p 18000 EXPIRE "frosty:cooldown:count" $((ENHANCED_COOLDOWN * 2)) >/dev/null
  
  # Apply enhanced cooldown
  redis-cli -p 18000 set "claude_restart_cooldown" 1 EX $ENHANCED_COOLDOWN >/dev/null
  exit 0
fi
...
echo "🔍 Processing wake signals..." >&2
if [ -f "/nfs/projects/claude-code-Tmux-Orchestrator/wake_signal_integration.sh" ]; then
    bash /nfs/projects/claude-code-Tmux-Orchestrator/wake_signal_integration.sh
    wake_exit_code=$?
    if [ $wake_exit_code -eq 2 ]; then
        echo "⚡ Wake signal triggered immediate restart" >&2
        exit 2
    fi
fi
